...
# ----------------------------------------
def perform_henm(cartesian_covariance_matrix,average_node_positions,output_directory,guess=None,max_iterations=100,alpha=0.1,kBT = 0.592,threshold=1e-4,distance_cutoff = 9999.,output_step=10):
        """
        """
        # ----------------------------------------
        ...
        dist_cutoff_mask = (x0 < inverse_distance_cutoff).astype(float)
        ...
        np.savetxt(output_directory + 'target_residual.dat',targetResidual,fmt='%.8f')
        ...
        while step < max_iterations and converged == 'False':
                ...
                if step%output_step == 0: 
                    np.savetxt(output_directory + '%05d.covar3N.dat'%(step),covar3N,fmt='%.8f')
                ...
                # save diffMatrix 
                if step%output_step == 0: 
                        np.savetxt(output_directory + '%05d.diffMatrix.dat'%(step),diffMatrix,fmt='%.8f')

                # check to see if converged; which is very unlikely to ever occur...
                dev = np.linalg.norm(diffMatrix)
                if dev < threshold:
                        converged = 'True'
                else: # update Hessian
                    hessian += alpha * diffMatrix
                    # distance cutoff and positive value removal applied 
